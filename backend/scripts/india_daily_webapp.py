...
fno_list = ['360ONE', 'AARTIIND', 'ABB', 'ABCAPITAL', 'ABFRL', 'ACC', 'ADANIENSOL', 'ADANIENT', 'ADANIGREEN', 'ADANIPORTS', 'ALKEM', 'AMBER', 'AMBUJACEM', 'ANGELONE', 'APLAPOLLO', 'APOLLOHOSP', 'ASHOKLEY', 'ASIANPAINT', 'ASTRAL', 'ATGL', 'AUBANK', 'AUROPHARMA', 'AXISBANK', 'BAJAJ-AUTO', 'BAJAJFINSV', 'BAJFINANCE', 'BALKRISIND', 'BANDHANBNK', 'BANKBARODA', 'BANKINDIA', 'BDL', 'BEL', 'BHARATFORG', 'BHARTIARTL', 'BHEL', 'BIOCON', 'BLUESTARCO', 'BOSCHLTD', 'BPCL', 'BRITANNIA', 'BSE', 'BSOFT', 'CAMS', 'CANBK', 'CDSL', 'CESC', 'CGPOWER', 'CHAMBLFERT', 'CHOLAFIN', 'CIPLA', 'COALINDIA', 'COFORGE', 'COLPAL', 'CONCOR', 'CROMPTON', 'CUMMINSIND', 'CYIENT', 'DABUR', 'DALBHARAT', 'DELHIVERY', 'DIVISLAB', 'DIXON', 'DLF', 'DMART', 'DRREDDY', 'EICHERMOT', 'ETERNAL', 'EXIDEIND', 'FEDERALBNK', 'FORTIS', 'GAIL', 'GLENMARK', 'GMRAIRPORT', 'GODREJCP', 'GODREJPROP', 'GRANULES', 'GRASIM', 'HAL', 'HAVELLS', 'HCLTECH', 'HDFCAMC', 'HDFCBANK', 'HDFCLIFE', 'HEROMOTOCO', 'HFCL', 'HINDALCO', 'HINDCOPPER', 'HINDPETRO', 'HINDUNILVR', 'HINDZINC', 'HUDCO', 'ICICIBANK', 'ICICIGI', 'ICICIPRULI', 'IDEA', 'IDFCFIRSTB', 'IEX', 'IGL', 'IIFL', 'INDHOTEL', 'INDIANB', 'INDIGO', 'INDUSINDBK', 'INDUSTOWER', 'INFY', 'INOXWIND', 'IOC', 'IRB', 'IRCTC', 'IREDA', 'IRFC', 'ITC', 'JINDALSTEL', 'JIOFIN', 'JSL', 'JSWENERGY', 'JSWSTEEL', 'JUBLFOOD', 'KALYANKJIL', 'KAYNES', 'KEI', 'KFINTECH', 'KOTAKBANK', 'KPITTECH', 'LAURUSLABS', 'LICHSGFIN', 'LICI', 'LODHA', 'LT', 'LTF', 'LTIM', 'LUPIN', 'M&M', 'M&MFIN', 'MANAPPURAM', 'MANKIND', 'MARICO', 'MARUTI', 'MAXHEALTH', 'MAZDOCK', 'MCX', 'MFSL', 'MGL', 'MOTHERSON', 'MPHASIS', 'MUTHOOTFIN', 'NATIONALUM', 'NAUKRI', 'NBCC', 'NCC', 'NESTLEIND', 'NHPC', 'NMDC', 'NTPC', 'NYKAA', 'OBEROIRLTY', 'OFSS', 'OIL', 'ONGC', 'PAGEIND', 'PATANJALI', 'PAYTM', 'PEL', 'PERSISTENT', 'PETRONET', 'PFC', 'PGEL', 'PHOENIXLTD', 'PIDILITIND', 'PIIND', 'PNB', 'PNBHOUSING', 'POLICYBZR', 'POLYCAB', 'POONAWALLA', 'POWERGRID', 'PPLPHARMA', 'PRESTIGE', 'RBLBANK', 'RECLTD', 'RELIANCE', 'RVNL', 'SAIL', 'SBICARD', 'SBILIFE', 'SBIN', 'SHREECEM', 'SHRIRAMFIN', 'SIEMENS', 'SJVN', 'SOLARINDS', 'SONACOMS', 'SRF', 'SUNPHARMA', 'SUPREMEIND', 'SYNGENE', 'TATACHEM', 'TATACOMM', 'TATACONSUM', 'TATAELXSI', 'TATAMOTORS', 'TATAPOWER', 'TATASTEEL', 'TATATECH', 'TCS', 'TECHM', 'TIINDIA', 'TITAGARH', 'TITAN', 'TORNTPHARM', 'TORNTPOWER', 'TRENT', 'TVSMOTOR', 'ULTRACEMCO', 'UNIONBANK', 'UNITDSPR', 'UNOMINDA', 'UPL', 'VBL', 'VEDL', 'VOLTAS', 'WIPRO', 'YESBANK', 'ZYDUSLIFE']

new_fno_list = []

for i in fno_list:
  try:
    new_download = yf.download(i + ".NS", timeout=540)
...
def target_sl_buy(symbol):
    ...
    try:
        ...
        while mean_prices - (standard_dev/4) > 0:
...
def target_sl_sell(symbol):
    ...
    try:
        ...
        mean_prices = -(np.mean(returns)) if np.mean(returns) > 0 else np.mean(returns)
        standard_dev = (np.std(returns))
...
def handlerData(symbol):
    df = yf.download(f"{symbol}.NS", timeout=540)
                ...
                if velocity > 25:
                    if change_volume > 25:
                        ...
                        try:
                            ...
                            df = yf.download(symbol + ".NS", timeout=540)
                        ...
                        try:
                            if (bos["RECOMMENDATION"] == "BUY") or (bos["RECOMMENDATION"] == "STRONG_BUY"):
                                ...
                                if ((close == open_stock) and (close == high)) or ((close == open_stock) and (close == low)) or ((low == open_stock) and (high == close)):
                                    ...
                                    if indicator["change"] >= 2.0 and (close - open_stock > 0) and (((high - close)/high) < 0.0025):
                                        if ((abs(moving_average_20 - low) / close) < 0.0015) and (close > moving_average_20):
...
def indian_stocks():
  ...
  try:
    ...
    for j in range(n):
        ...
        try:
            ...
            try:
                ...
                bos = recommendation_summary_technical(f"{stock_list[j]}.NS")
                if (bos["RECOMMENDATION"] == "BUY") or (bos["RECOMMENDATION"] == "STRONG_BUY"):
                    strong_buy.append(stock_list[j])
                elif (bos["RECOMMENDATION"] == "SELL") or (bos["RECOMMENDATION"] == "STRONG_SELL"):
                    strong_sell.append(stock_list[j])
    ...
    def level_2():
        ...
        for i in range(o):
            ...
            try:
                try:
                    output = TA_Handler(symbol=strong_sell[i],
                                    screener="india",
                                    exchange="NSE",
                                    interval=Interval.INTERVAL_1_DAY)
                    ind = output.get_analysis().indicators
                    if ind["change"] <= -2.0:
                        strong_sell_level_2.append(strong_sell[i])
                except Exception as e:
                    ind = handlerData(strong_sell[i])
                    if ind["change"] <= -2.0:
                        strong_sell_level_2.append(strong_sell[i])
            except Exception as e:
                (traceback.print_exc())
  ...
  file_path = 'India_equity_Daily.csv'
  ...
  if len(df_earnings_data) > 0:
      ...
      print("No recommendations for today - email not sent.")
